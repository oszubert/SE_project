...
//  srednia fps
    ...
    if (!ImGui::SFML::Init(window)) {
        cerr << "Nie udalo sie zainicjowac ImGUI" << endl;
        return 1;
    }
...
//        float gameTime=gameClock.restart().asSeconds(); // Zegar gry
        sf::Time gameTime=gameClock.restart(); // Zegar gry
        ImGui::SFML::Update(window, gameTime);

        // Wyswietla klatki na sekunde w tytule okna
        avgFps = 0.9 * avgFps + 0.1 * (1/gameTime.asSeconds());
        ...
        if(gamestate==State::Game){
            ...
            player.update(gameTime.asSeconds(), mouse.getPosition(window)); // Aktualizacja pozycji gracza
